...
def generate_audit_report(package_name: str, risk_score: float, author_age: int, explanation: str) -> str:
    ...
    """
    try:
        print(f"Generating PDF for {package_name}...")
        ...
        pdf.cell(0, 10, f"Risk Score: {risk_score:.2f}/1.0", new_x="LMARGIN", new_y="NEXT")
        ...
        # Handle multi-line explanation safely
        safe_explanation = explanation.encode('latin-1', 'replace').decode('latin-1')
        pdf.multi_cell(0, 10, f"AI Analysis Summary:\n{safe_explanation}")
        
        pdf.ln(20)
        pdf.set_font("helvetica", size=10)
        pdf.cell(0, 10, "Generated by SupplyGuard MCP Agent", new_x="LMARGIN", new_y="NEXT")

        # 3. Save to Bytes
        pdf_bytes = pdf.output()

        # 4. Upload to transfer.sh (Robust for CI/CD environments)
        print("Uploading to transfer.sh...")
        filename = f"{package_name}_report.pdf"
        url = f"https://transfer.sh/{filename}"
        
        # Use httpx (sync) to upload
        with httpx.Client() as client:
            response = client.put(url, content=pdf_bytes)
            ...
            if response.status_code == 200:
                ...
                return f"Error: PDF created, but upload failed (Status: {response.status_code})"

    except Exception as e:
        print(f"CRITICAL ERROR: {str(e)}")
        return f"System Error during PDF generation: {str(e)}"
